...
export default (bot: Telegraf<MyContext>) => {
    ...
    const renderOrderHistory = async (ctx: MyContext, page: number) => {
        ...
        transactions.forEach((trx, index) => {
            ...
            caption += `*${(safePage - 1) * PER_PAGE + index + 1}. ${trx.serviceName}* — ${trx.countryName}\n`;
        ...
        const keyboard = [
            makePaginationButtons('history_p', safePage, totalPages),
    ...
    const renderDepositHistory = async (ctx: MyContext, page: number) => {
        const dbUser = ctx.dbUser;
        if (!dbUser) return;

        const totalDeposits = await Deposit.countDocuments({ user: dbUser._id });

        if (totalDeposits === 0) {
            return ctx.editMessageCaption('📭 *Anda belum punya riwayat deposit.*', {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [[Markup.button.callback('🏠 Menu Utama', 'back_home')]] }
            });
        }
        ...
        let caption = `💳 *Riwayat Deposit Kamu*\n📄 Halaman ${safePage} dari ${totalPages}\n\n`;

        deposits.forEach((depo, index) => {
            let statusEmoji = '⏳';
            if (depo.status === 'success') statusEmoji = '✅';
            if (depo.status === 'canceled') statusEmoji = '❌';

            const tanggal = depo.createdAt.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' });

            caption += `*${(safePage - 1) * PER_PAGE + index + 1}. Deposit ID* \`${depo.depositId}\`\n`;
            caption += `💸 *Nominal:* Rp${depo.amount.toLocaleString('id-ID')}\n`;
            caption += `🧾 *Fee:* Rp${depo.fee.toLocaleString('id-ID')}\n`;
            caption += `💰 *Total Bayar:* Rp${depo.total.toLocaleString('id-ID')}\n`;
            caption += `🗓️ *Tanggal:* ${tanggal}\n`;
            caption += `🚥 *Status:* ${statusEmoji} ${depo.status.toUpperCase()}\n`;
            caption += `━━━━━━━━━━━━━━━━━━\n`;
        });

        const keyboard = [
            makePaginationButtons('history_d_p', safePage, totalPages),
            [Markup.button.callback('🏠 Menu Utama', 'back_home')]
        ];
    ...
    bot.action(['history_orderbot', 'user_history', /^history_p_(\d+)$/], async (ctx) => {
        try {
            ...
            await renderOrderHistory(ctx, page);
    ...
    bot.action(['history_deposit_user', 'user_history_deposit', /^history_d_p_(\d+)$/], async (ctx) => {
        try {
            const isPaging = !!(ctx.match && ctx.match[0].startsWith('history_d_p_'));
            const page = isPaging ? parseInt(ctx.match[1]) : 1;

            if (!isPaging) {
                await ctx.editMessageCaption('⏳ *Memuat riwayat deposit Anda...*', { parse_mode: 'Markdown' });
            }

            await renderDepositHistory(ctx, page);
        } catch (error: any) {
            console.error('Deposit History Error:', error.message);
            await ctx.editMessageCaption('❌ *Terjadi kesalahan saat memuat riwayat deposit.*', {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [[Markup.button.callback('🏠 Menu Utama', 'back_home')]] }
            });
        }
